...
export default function App() {
  ...
  // Credenciales de Entorno (o directas si fallan las variables)
  ...
  const handleAuth = async (e) => {
    ...
    try {
      if (authMode === 'register') {
        ...
        setUser({ nombre: localStorage.getItem('kolma_user_name') || formData.email.split('@')[0], email: formData.email, direccion: localStorage.getItem('kolma_user_address'), telefono: localStorage.getItem('kolma_user_phone'), id: token });
        setIsAuthOpen(false);
  ...
  return (
    <div style={{ backgroundColor: '#F9FAFB', minHeight: '100vh', fontFamily: 'system-ui, -apple-system, sans-serif', color: '#1F2937', paddingBottom: '100px', overflowX: 'hidden' }}>
      ...
      {activeTab === 'inicio' && (
        <>
          {/* BANER OSCURO PREMIUM (Search Bar Corregida) */}
