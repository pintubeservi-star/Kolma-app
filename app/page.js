...
// --- ICONOGRAFÍA SVG NATIVA (Cero dependencias) ---
const Icons = {
...
export default function KolmaRD() {
  ...
  const confirmOrder = async () => {
    ...
    } catch (e) { showToast("Error en Shipday"); } finally { setLoading(false); }
  };
    ...
    <div className="space-y-4 pb-32">
        ...
        <>
          <div className="relative h-44 w-full rounded-[32px] overflow-hidden shadow-lg mt-2">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent p-6 flex flex-col justify-end">
              <span className="bg-orange-500 text-white px-3 py-1 rounded-xl text-[8px] font-black uppercase tracking-widest w-fit mb-2">Fresco de Cotuí</span>
      ...
      <div className="flex space-x-3 overflow-x-auto no-scrollbar py-2">
        {['Todos', 'Carnes', 'Granos', 'Lácteos'].map(c => (
          <button key={c} onClick={() => setActiveCategory(c)} className={`px-6 py-3 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${activeCategory === c ? 'bg-red-600 text-white shadow-xl' : 'bg-gray-100 text-gray-400'}`}>{c}</button>
        ))}
      </div>
        ...
        {products.filter(p => (activeCategory === 'Todos' || p.category === activeCategory) && p.name.toLowerCase().includes(searchTerm.toLowerCase())).map(p => (
          <div key={p.id} onClick={() => setSelectedProduct(p)} className="flex items-center bg-white p-3 rounded-[28px] border border-gray-50 shadow-sm cursor-pointer active:scale-[0.98] transition-all">
            ...
            <div className="ml-4 flex-1">
              <h4 className="text-[12px] font-black text-gray-900 uppercase leading-none mb-1">{p.name}</h4>
    ...
    <div className="min-h-screen bg-white text-[#0D1117] font-sans selection:bg-red-500 overflow-x-hidden">
      ...
      {view !== 'tracking' && (
        <header className="sticky top-0 z-[80] bg-white/95 backdrop-blur-xl px-6 py-4 border-b border-gray-50">
          ...
          <div className="relative">
            ...
            <input type="text" placeholder="¿QUÉ NECESITAS FRESCO?" className="w-full bg-gray-100 py-3.5 pl-12 pr-6 rounded-2xl text-[11px] font-black outline-none uppercase tracking-widest border-2 border-transparent focus:border-red-500 transition-all" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
          </div>
        </header>
      )}
      ...
      <main className="max-w-md mx-auto px-6">
        {loading ? (
          ...
          <motion.div key={view} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            {view === 'home' && <HomeView />}
            {view === 'checkout' && (
                <div className="pt-6 pb-32 space-y-8">
            ...
            {view === 'orders' && (
              <div className="pt-8 space-y-8 pb-32">
                ...
                <div onClick={() => setView('tracking')} className="bg-white p-8 rounded-[40px] border-2 border-red-600 shadow-2xl cursor-pointer active:scale-[0.98]">
                  <div className="flex justify-between items-center mb-6">
            ...
            {view === 'profile' && (user ? (
              <div className="pt-8 space-y-10 pb-32">
                ...
                <button onClick={() => { setUser(null); localStorage.removeItem('kolmard_user'); setView('home'); }} className="w-full p-6 text-gray-400 font-black text-[11px] uppercase tracking-widest bg-gray-50 rounded-2xl flex items-center justify-center gap-2"><Icons.LogOut size={16} /> Cerrar Sesión</button>
              </div>
            ) : (
              <div className="pt-12 px-2">
                ...
                <form onSubmit={handleAuth} className="space-y-5">
                  {authMode === 'register' && (
                    <>
                      <div className="bg-gray-50 rounded-2xl flex items-center px-6 py-5 space-x-4"><Icons.User size={20} className="text-red-600" /><input type="text" placeholder="NOMBRE" className="bg-transparent font-black w-full outline-none uppercase" value={formData.name} onChange={e=>setFormData({...formData, name: e.target.value})} /></div>
                      <div className="bg-gray-50 rounded-2xl flex items-center px-6 py-5 space-x-4"><Icons.MapPinned size={20} className="text-red-600" /><input type="text" placeholder="DIRECCIÓN" className="bg-transparent font-black w-full outline-none uppercase" value={formData.address} onChange={e=>setFormData({...formData, address: e.target.value})} /></div>
                      <div className="bg-gray-50 rounded-2xl flex items-center px-6 py-5 space-x-4"><Icons.Phone size={20} className="text-red-600" /><input type="text" placeholder="TELÉFONO" className="bg-transparent font-black w-full outline-none uppercase" value={formData.phone} onChange={e=>setFormData({...formData, phone: e.target.value})} /></div>
                    </>
                  )}
                  <div className="bg-gray-50 rounded-2xl flex items-center px-6 py-5 space-x-4"><Icons.Mail size={20} className="text-red-600" /><input type="email" placeholder="EMAIL" className="bg-transparent font-black w-full outline-none uppercase" value={formData.email} onChange={e=>setFormData({...formData, email: e.target.value})} /></div>
                  <div className="bg-gray-50 rounded-2xl flex items-center px-6 py-5 space-x-4 relative">
                  ...
                  <button type="submit" className="w-full h-20 bg-red-600 text-white rounded-[32px] font-black uppercase italic shadow-2xl text-xl">
                    {loading ? '...' : authMode === 'login' ? 'Entrar' : 'Registrarme'}
                  </button>
                </form>
                <button onClick={() => setAuthMode(authMode === 'login' ? 'register' : 'login')} className="w-full text-center mt-10 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">
      ...
      {view !== 'tracking' && (
        <nav className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-3xl border-t border-gray-100 px-8 py-6 flex justify-between items-center z-[90] pb-10">
          <button onClick={()=>setView('home')} className={`flex flex-col items-center space-y-1.5 ${view === 'home' ? 'text-red-600 scale-110' : 'text-gray-300'}`}><Icons.ShoppingBag size={24} /><span className="text-[9px] font-black uppercase italic">Inicio</span></button>
          <button onClick={()=>setView('orders')} className={`flex flex-col items-center space-y-1.5 ${view === 'orders' ? 'text-red-600 scale-110' : 'text-gray-300'}`}><Icons.Truck size={24} /><span className="text-[9px] font-black uppercase italic">Pedidos</span></button>
          ...
          <button onClick={()=>setView('offers')} className={`flex flex-col items-center space-y-1.5 ${view === 'offers' ? 'text-red-600 scale-110' : 'text-gray-300'}`}><Icons.Tag size={24} /><span className="text-[9px] font-black uppercase italic">Ofertas</span></button>
          <button onClick={()=>setView('profile')} className={`flex flex-col items-center space-y-1.5 ${view === 'profile' ? 'text-red-600 scale-110' : 'text-gray-300'}`}><Icons.User size={24} /><span className="text-[9px] font-black uppercase italic">Perfil</span></button>
        </nav>
      )}
        ...
        {view === 'tracking' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[120] bg-white flex flex-col">
            <div className="absolute top-12 left-6 right-6 z-10 flex items-center space-x-4">
              <button onClick={() => setView('orders')} className="w-14 h-14 bg-white rounded-2xl shadow-2xl flex items-center justify-center text-black active:scale-90"><Icons.ArrowLeft size={24} /></button>
            ...
            <div className="bg-white rounded-t-[56px] px-10 pt-10 pb-14 shadow-[0_-25px_60px_rgba(0,0,0,0.15)] relative z-20">
              <div className="w-16 h-2 bg-gray-100 rounded-full mx-auto mb-10"></div>
              <div className="flex justify-between items-start mb-10">
                <div><h3 className="text-3xl font-black italic uppercase text-red-600 leading-none">Llega en 12 min</h3></div>
                <a href={`tel:${user?.phone}`} className="w-14 h-14 bg-red-600 rounded-2xl flex items-center justify-center text-white shadow-xl"><Icons.Phone size={24}/></a>
              </div>
            </div>
          </motion.div>
        )}
      ...
      <AnimatePresence>
        {selectedProduct && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[150] bg-black/80 backdrop-blur-md flex items-end">
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="bg-white w-full rounded-t-[48px] overflow-hidden shadow-2xl">
              <div className="relative h-80 w-full group">
                <img src={selectedProduct.img} className="w-full h-full object-cover" alt={selectedProduct.name} />
                <button onClick={() => setSelectedProduct(null)} className="absolute top-8 right-8 bg-black/20 backdrop-blur-xl text-white p-3 rounded-full hover:bg-red-600"><Icons.X size={24} /></button>
              </div>
              <div className="px-10 pb-12 pt-8">
                ...
                }} className="w-full bg-red-600 text-white h-20 rounded-[32px] font-black uppercase mt-10 italic shadow-2xl text-lg">Agregar a mi canasta</button>
              </div>
            </motion.div>
          </motion.div>
        )}
